cluster_ts <-
  function(capacity_factors_subset,
           erroneous,
           nmb_clusters,
           grouping,
           colors) {
    data_clustering <- capacity_factors_subset %>%
      ...
      mutate(Capacity_Factor_Normalized = Capacity_Factor_mean / max(Capacity_Factor_mean, na.rm =
                                                                       TRUE)) %>%
      ungroup()  %>%
    ...
    data_clustering_mat <-
      data_clustering[, 2:ncol(data_clustering)] %>% as.matrix()
    ...
    p <- ret %>%
      ...
      ggplot(aes(x = Time, y = Capacity_Factor)) + geom_line(aes(color = Location)) +
      facet_wrap(. ~ Cluster) +
    ...
    selection <- tibble(
      Location = c(erroneous, tracking, non_tracking),
      Type = c(
        rep("erroneous", length(erroneous)),
...
quality_assess_selection <- function(selection) {
  ...
  final_tab$Resolution <- factor(
    final_tab$Resolution,
  ...
  final_tab_join <-
    full_join(final_tab, t4, by = c("Installation" = "NOMBRE")) %>%
    mutate(Set = set) %>%
...
classify_erroneous <-
  function(capacity_factors_length_series,
           min_n,
           ylim) {
    capacitiy_factors_length_series <- capacity_factors_classified %>%
      na.omit() %>%
      group_by(Location) %>%
      summarize(length = summarize_length(class),
                set_ = mean(set_, na.rm = TRUE))
    ...
    set_remove_0 <-
      capacitiy_factors_length_series %>% filter(length > min_n) %>% dplyr::select(Location)
...
find_erroneous <- function(capacity_factors_subset) {
  ...
  capacity_factors_classified <-
    capacity_factors_subset_mean_yearly %>%
    mutate(class = Capacity_Factor < 0.5 * yearly_mean)
  
  
  
  set_remove_rule1 <-
    classify(capacity_factors_classified, 70, c(0, 70))
  
  
  
  #Rule_2: series of values at less than 70% of monthly mean
  capacity_factors_subset_mean_monthly <-
  ...
  erroneous <-
    union(set_remove_rule1, set_remove_rule2) %>% unlist()
...
plot_classification_procedure <-
  function(ts, loc, thresholds = TRUE) {
    yearly_cap <- ts %>%
      na.omit() %>%
      group_by(Location) %>%
      mutate(yearly_mean = mean(Capacity_Factor, na.rm = TRUE)) %>%
    ...
    p <- ts %>%
      ...
      filter(Time > ymd_hm("2016-01-01 23:00")) %>%
      filter(Time < ymd_hm("2019-01-01 00:00")) %>%
      mutate(is_yearly_err = ifelse(class_type_y &
                                      class_length_y > 70, Capacity_Factor, NA)) %>%
      mutate(is_monthly_err = ifelse(class_type_m &
                                       class_length_m > 70, Capacity_Factor, NA)) %>%
      ggplot(aes(x = Time, y = Capacity_Factor)) +
      geom_line(col = COLORS3[2], size = 0.1) +
      geom_line(
        data = monthly_cap,
        aes(x = Time, y = monthly_mean),
        col = COLORS3[1],
        linetype = 2
      ) +
      ylab("Capacity Factor") +
      geom_hline(
        yintercept = capacity_factors_classified_y$yearly_mean[1],
        linetype = 2,
        size = 1.2,
        col = COLORS3[3]
      )
    ...
    if (thresholds) {
      p <- p +
        geom_line(aes(x = Time, y = is_yearly_err),
                  col = COLORS3[3],
                  size = 0.1) +
        geom_line(aes(x = Time, y = is_monthly_err),
                  col = COLORS3[1],
                  size = 0.1)
    }
    p
...
load_indicators <-
  function(merra2_tracking_file = "data/indicators_merra2_tracking.csv",
    ...
    final_tab_join <-
      full_join(final_tab, selection, by = c("Installation" = "Location")) %>%
      mutate(Set = set)
...
violin_plot <- function(selection) {
  ts_era5l_optimal <-
    read_csv("timeseries_capacity_factors_pv_optimal_era5l.csv") %>%
  ...
  ts_era5l_tracking <-
    read_csv("timeseries_capacity_factors_pv_tracking_era5l.csv") %>%
    gather(Installation, Capacity_Factor,-X1) %>%
    dplyr::select(DateTime = X1, Installation, Capacity_Factor) %>%
    full_join(selection, by = c("Installation" = "Location")) %>%
    filter(Type == "tracking") %>%
  ...
  ts_merra2_optimal <-
    read_csv("timeseries_capacity_factors_pv_optimal_merra2.csv")  %>%
    gather(Installation, Capacity_Factor,-X1) %>%
    dplyr::select(DateTime = X1, Installation, Capacity_Factor) %>%
    full_join(selection, by = c("Installation" = "Location")) %>%
    filter(Type == "non-tracking") %>%
  ...
  ts_merra2_tracking <-
    read_csv("timeseries_capacity_factors_pv_tracking_merra2.csv") %>%
    gather(Installation, Capacity_Factor,-X1) %>%
    dplyr::select(DateTime = X1, Installation, Capacity_Factor) %>%
    full_join(selection, by = c("Installation" = "Location")) %>%
    filter(Type == "tracking") %>%
  ...
  ts_reference <-
    ts_reference %>% gather(Location, Capacity_Factor, -Timestamp) %>%
  ...
  all_capacity_factors_join <- bind_rows(ts_era5l_optimal,
                                         ...
                                         ts_merra2_optimal,
                                         ts_merra2_tracking) %>%
    full_join(ts_reference,
              by = c("Installation" = "Location", "DateTime" = "Timestamp"))
  ...
  tt <- all_capacity_factors_join %>%
    ...
    na.omit() %>%
    mutate(Reference = Capacity_Factor.y,
           Capacity_Factor = Capacity_Factor.x,
           Source = Source.x)
  
  grp <-
    cut(tt$Reference, breaks = hist(tt$Reference, breaks = 5)$breaks)
...
density_plot <-
  function(files = c(
    "data/timeseries_capacity_factors_pv_optimal_era5l.csv",
  ...
  file_name = "Figure-4-density.png") {
    ...
    ts_reference_optimal <-
      read_csv(files[5], col_types = paste0(c("?", rep("d", 57)), collapse = ""))
    names(ts_reference_optimal)[1] <- "Timestamp"
    ts_reference_optimal <-
      ts_reference_optimal %>% gather(Location, Capacity_Factor, -Timestamp) %>%
      mutate(Source = "Reference", Type = "non-tracking")
    
    ts_reference_tracking <-
      read_csv(files[6], col_types = paste0(c("?", rep("d", 57)), collapse = ""))
    names(ts_reference_tracking)[1] <- "Timestamp"
    ts_reference_tracking <-
      ts_reference_tracking %>% gather(Location, Capacity_Factor, -Timestamp) %>%
      mutate(Source = "Reference", Type = "tracking")
    
    ts_reference <-
      bind_rows(ts_reference_optimal, ts_reference_tracking)
    ...
    p <- tt %>%
      ...
      scale_fill_gradientn(
        colours = c("white", COLORS3[1], COLORS3[1], COLORS3[1], COLORS3[1]),
        trans = "identity"
      ) +
      #COLORS3[1]
...
calculate_capacity_factor_simulation_quality <- function() {
  ...
  capacity_factors_gather_tracking_merra2 <-
  ...
  capacity_factors_mean <-
    full_join(
      capacity_factors_gather_tracking,
      capacity_factors_gather_fixed,
      by = c("Installation" = "Installation")
    ) %>%
    mutate(tracking = mean_cap.x,
           fixed = mean_cap.y,
           Source = "era5-land")
  ...
  return(mean_cor_joined)
  
}

calculate_irr_diff_factor_simulation_quality <- function() {
  clear_sky_era5 <-
    full_join(clear_sky,
              era5_irridiance,
              by = c("Time" = "Time", "Location" = "Location"))
  
  clear_sky_era5_diff <-
    clear_sky_era5 %>% mutate(diff = abs(Irridiance.x - Irridiance.y))  %>%
...
correlation_one_out_mean <- function(set_, mean_cor_joined) {
  fixed_mean_cor_joined <- mean_cor_joined %>%
    filter(Set == set_)
  
  names <-
    fixed_mean_cor_joined$Installation %>% unique() %>% unlist()
  correlations_one_out <- tibble(Name = names, Cor = 0)
  i <- 1
  for (name in names) {
    cor <- fixed_mean_cor_joined %>%
      filter(Installation != name) %>%
      group_by(Set) %>%
      summarize(cor = cor(mean_cap, Value))
    
    correlations_one_out$Cor[i] <- cor$cor
    
    i <- i + 1
  }
  
  return(correlations_one_out)
}

correlation_one_out_diff <- function(set_, mean_cor_joined) {
  fixed_mean_cor_joined <- mean_cor_joined %>%
    filter(Set == set_)
